...
```{r}
funcionSimulaEstrVar(gobernador2015, n, M, df.distritos)
```
...
intervaloestrprop <- simulacionESTR.prop %>% 
  summarise(PRI= mean(PRI), PAN=mean(PAN))%>%
  gather("Partido", "Promedio") %>%
  mutate(Mínimo=Promedio-(3*sqrt(as.double(resumen2$`Estr prop`))),
         Máximo=Promedio+(3*sqrt(as.double(resumen2$`Estr prop`)))) %>%
  filter(Partido!="PRD")
