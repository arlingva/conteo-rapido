...
```{r}
funcionSimulaEstrSimple02 <- function(df, n, M, df.distritos){
  ...
  for(i in 1:M){
    ...
    resultado[i, ] <- c(
      estPRI = df.distritos$N.dist %*% varMAS(df.temporal$PRI) / N,
...
df <-simulacionESTR.simple
varEstESTR <- function(x, Ni){
  ni
}
